...
def avg_IOU(X, centroids):
    ...
    for i in range(X.shape[0]):
        # note IOU() will return array which contains IoU for
        # each centroid and X[i] // slightly ineffective,
        # but I am too lazy
...
def main(argv):
    ...
    for idx, filename in enumerate(tqdm(lines)):
        filename = filename.replace('/images', './ln_data/other/images/mscoco/annotations')
        # line = line.replace('img1','labels')
        # line = line.replace('JPEGImages', 'labels')

        filename = filename.replace('.jpg', '.txt').replace(".png", ".txt")
